...
Autores<-function(){
  #############OOOOOOOOO############
  oh<-c(1,1,3,3,1,1)
  ov<-c(1,5,5,1.1,1.1,1)
  ##############LLLLLLLL###############
  lh<-c(1,1,1,3,3,1)
  lv<-c(1,5,1.1,1.1,1,1)
  ##############GGGGGGGG###############
  gh<-c(1,1,3,1,1,3,3,2,2)
  gv<-c(1,5,5,5,1.1,1.1,3,3,1)
  ##############AAAAAAAAA###############
  ah<-c(1,1,3,3,1,3,3)
  av<-c(1,5,5,3,3,3,1)
  ##########YYYYYYYYYYYYY##########
  ...
  ##########uuuuuuuuuuu##########
  uh<-c(1,1,1,3,3,3,1)
  uv<-c(1,5,1.1,1.1,5,1,1)
  #########EEEEEEEEEEEEEEE############
  eh<-c(1,1,3,1,1,2.5,1,1,3,3,1)
  ev<-c(1,5,5,5,3,3,3,1.1,1.1,1,1)
  ##########IIIIIIIIIIIIIIII##########
  ih<-c(1,3,2,2,1,3,2,2)
  iv<-c(1,1,1,5,5,5,5,1)
  ##########NNNNNNNNNNNNNNN###########
  nh<-c(1,1,3,3,3)
  nv<-c(1,5,1,5,1)
  ##############RRRRRRRRRR###############
  ...
  rv<-c(1,5,5,3,3,1)
  ##########FFFFFFFFFFFFFFF##########
  fh<-c(1,1,3,1,1,2.5,1,1)
  fv<-c(1,5,5,5,3,3,3,1)
  xx<-c(1:3)
  xxv<-c(1,1,1)
  YEFRIh<-c(yh+2,eh+7,fh+12,rh+17,ih+22,xx+27)
  YEFRIv<-c(yv,ev,fv,rv,iv,xxv)
  cc<-c(1:length(YEFRIv))
  YEFRI<-data.frame(YEFRIh,YEFRIv,cc)
  y1<-ggplot(YEFRI, aes(x=YEFRIh, y=YEFRIv))+geom_line(color=4,lwd=5)+geom_point(zise=8)+
    transition_reveal(cc)
  ##########YEINER################
  YEINERh<-c(yh+2,eh+7,ih+12,nh+17,eh+22,rh+27,xx+32)
  YEINERv<-c(yv,ev,iv,nv,ev,rv,xxv)
  ccy<-c(1:length(YEINERv))
  YEINER<-data.frame(YEINERh,YEINERv,ccy)
  y2<-ggplot(YEINER, aes(x=YEINERh, y=YEINERv))+geom_line(color=3,lwd=5)+geom_point(zise=8)+
    transition_reveal(ccy)
  ##############YULE####################
  YULEh<-c(yh+2,uh+7,lh+12,eh+17,xx+22)
  YULEv<-c(yv,uv,lv,ev,xxv)
  ccu<-c(1:length(YULEv))
  YULE<-data.frame(YULEh,YULEv,ccu)
  y3<-ggplot(YULE, aes(x=YULEh, y=YULEv))+geom_line(color=2,lwd=5)+geom_point(zise=8)+
    transition_reveal(ccu)
  #####OLGAAAAAA###########
  OLGAh<-c(oh+2,lh+7,gh+12,ah+17,xx+22)
  OLGAv<-c(ov,lv,gv,av,xxv)
  cco<-c(1:length(OLGAv))
  OLGA<-data.frame(OLGAh,OLGAv,cco)
  o1<-ggplot(OLGA, aes(x=OLGAh, y=OLGAv))+geom_line(color=5,lwd=5)+geom_point(zise=8)+
    transition_reveal(cco)
